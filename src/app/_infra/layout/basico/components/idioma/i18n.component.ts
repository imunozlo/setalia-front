      ...
      <ul nz-menu>
        @for (item of langs; track $index) {
          <li nz-menu-item [nzSelected]="item.code === curLangCode" (click)="change(item.code)">
            <span [attr.aria-label]="item.text">{{ item.abbr }}</span>
            {{ item.text }}
          </li>
        }
      </ul>
...
export class HeaderI18nComponent {
  ...
  change(lang: string): void {
    ...
    this.i18n.obtenerDadesIdioma(lang).subscribe((res: any[]) => {
      const datos = {};
      res.map(dada => {
        // @ts-ignore
        datos[dada.clau] = dada.descripcion;
      });
      this.i18n.use(lang, datos);
