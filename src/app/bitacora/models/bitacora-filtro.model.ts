...
export class BitacoraFiltroModel extends BaseModel {
  titulo: string;
  provincia: string[];
  municipio: string[];
  fecha: Date[];
  ...
  iniciarFiltro(columnas: ColumnaInterface[]) {
    ...
    this.setaId = UtilsFiltros.obtenerValorFiltro(columnas, 'setaId');
    if (!this.pageIndex) this.pageIndex = 1;
    if (!this.pageSize) this.pageSize = 15;
    return this;
  }
  ...
  deserializeFiltros(input: any, columnas: ColumnaInterface[]) {
    ...
    UtilsFiltros.asignarValorFiltro(columnas, this.setaId, 'setaId');
    if (!this.pageIndex) this.pageIndex = 1;
    if (!this.pageSize) this.pageSize = 15;
    return this.prepare();
  }
